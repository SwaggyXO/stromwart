...
class EvidenceCritic:
    def review(
        ...
        if finding.confidence >= 0.8 and finding.missing_evidence:
            reasons.append(
                "high-confidence finding conflicts with declared missing evidence"
            )

        if finding.recommended_action == "increase_cdn_capacity":
            telemetry_seen = any(
                result.call.name.value == "telemetry"
                and "throughput_mean_kbps" in result.output
                for result in results
            )
            if not telemetry_seen:
                reasons.append(
                    "capacity recommendation lacks telemetry evidence"
                )
